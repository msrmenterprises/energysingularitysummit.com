...
<!--<h4>Voting will be open soon.</h4>-->
<section class="main" style="margin-top: 10px;">
		<div class="inner">
		<div class="clearfix"></div>
		<p>&nbsp </p>
			<p style="font-size:15px;" class="cast_info">Click on<strong> 'VOTE NOW'</strong> and <strong>'Sign in through Twitter'</strong> to <strong>'Vote'</strong> for your favorite Nomination and help them in winning <strong>"ISGF Innovation Awards 2023"</strong>. </p>
			<p>Please note:<br/>
&nbsp  1. An individual can cast only one vote through a Twitter account.<br/>
&nbsp  2. Voting will be active till<b> 11:30 AM -04:00 PM 10 February 2023</b><br/>
...
if (!isset($_SESSION['access_token'])) {
	...
	?>
	<?php 
			$getProjectCategory = mysqli_query($conn, "select distinct(voting_category_backup_2022) as v_category from voting_category_backup_2022 order by id ASC");
			if(mysqli_num_rows($getProjectCategory)>0){
				
				while($result = mysqli_fetch_array($getProjectCategory)){ 
				
					$getProjectByCategory = mysqli_query($conn, "select * from voting_category_backup_2022 where voting_category_backup_2022 = '".$result['v_category']."'  order by id ASC");
				?>
					<tr><td colspan="5" style="color:#f60;"><strong><?php echo $result['v_category']; ?></strong></td><tr>
						...
						while($resultmain = mysqli_fetch_array($getProjectByCategory)){
							$sum = 0;
							$i++;
							$query = mysqli_query($conn, "select count(*) as count_main,vote_category,vote_categorymain  from voting_backup_2022 where vote_category='".$resultmain['nominee']."' and vote_categorymain = '".$result['v_category']."'");
	...
	$checkifalreadyvoted = mysqli_query($conn, "select * from voting_backup_2022 where screenname = '".$screenname."'");
	...
	<?php } else {
		$sQL = "insert into voting_backup_2022 set vote_category = '".$nomination."',vote_categorymain = '".$project_category."', name = '".$username."', screenname='".$screenname."', date_added='".date('Y-m-d H:i:s')."'";
		//echo $sQL; exit;
